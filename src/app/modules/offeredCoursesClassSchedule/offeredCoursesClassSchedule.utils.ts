...
const checkRoomAvailable = async (data: OfferedCoursesClassSchedule) => {
    const alreadyBookedRoomOnDay = await prisma.offeredCoursesClassSchedule.findMany({
        where: {
...
const checkFacultyAvailable = async (data: OfferedCoursesClassSchedule) => {
    const alreadyFaultyAssigned = await prisma.offeredCoursesClassSchedule.findMany({
        where: {
            dayOfWeek: data.dayOfWeek,
            faculty: {
                id: data.facultyId
